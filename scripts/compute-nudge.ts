/**
 * scripts/compute-nudge.ts
 *
 * CLI runner for computing the risk-nudge for a specific doctor (or all active doctors).
 * Usage:
 *   npx tsx scripts/compute-nudge.ts --email ardytcm@gmail.com
 *   npx tsx scripts/compute-nudge.ts --doctorId <uuid>
 *   npx tsx scripts/compute-nudge.ts --email ardytcm@gmail.com --force
 *
 * Requires: .env.local with NEXT_PUBLIC_SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY + DEEPSEEK_API_KEY
 * Run: node --env-file=.env.local node_modules/.bin/tsx scripts/compute-nudge.ts --email ...
 *      OR: npm run nudge -- --email ...
 */

import { createClient } from "@supabase/supabase-js";
import {
  computeNudgeForDoctor,
...
async function main() {
  console.log("=== Doctor Risk Nudge CLI ===\n");
  ...
  const t0 = Date.now();
  const result = await computeNudgeForDoctor(admin, targetId, { force });
  const elapsed = Date.now() - t0;

  console.log(`\nResult:`);
  console.log(`  status:     ${result.status}`);
  if (result.themeCount !== undefined) console.log(`  themes:     ${result.themeCount}`);
  console.log(`  elapsed:    ${elapsed}ms`);

  if (result.status === "computed") {
    console.log(`\n✓ Nudge row upserted for ${targetLabel}.`);
    console.log("  Verify: SELECT doctor_id, jsonb_array_length(themes), computed_at, source_last_record_at FROM doctor_risk_nudges;");
