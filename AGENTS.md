...
- **Version immutability**: Once committed, a version file must not be modified. If a prompt needs changing, create a new version file (`v1.7.ts`) and update the `latest` pointer. A `commitCount > 1` warning appears in `/admin/prompts` when this invariant is violated.
- **Admin browser**: `/admin/prompts` — build-time git history manifest at `src/lib/prompts/_history.generated.json`, regenerated by `scripts/build-prompt-history.mjs` on every `npm run build` and `npm run dev`. Shows all versions ever committed (including deleted files), commit metadata, source chip, and full prompt text. CI uses `fetch-depth: 0` to ensure full history is available during build.
- **Registry metadata helpers** (`src/lib/prompts/index.ts`): `envVarNameFor(key)`, `listPromptFamilies()`, `describeActiveVersion(key)` — used by the admin page to render the source chip and env var name without runtime git access.
