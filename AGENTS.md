...
Keep it in sync with shipped behavior.
...
- see completeness reminders before analysis goes too far
- receive supportive Chinese clinical review output
...
- Default to a continuous workbench: draft on top, analysis below.

## Branch And Delivery Rules

- Use a single-branch workflow on `main` unless the user explicitly asks otherwise.
- Do not leave major behavior only in a local draft state without saying so clearly.
...
- `针灸方案`: must include `穴位与操作` or sufficiently specific treatment method
...
- Keep setup/debug internals brief unless explicitly requested.
- When adding operational steps, keep them short and in the right sequence.
